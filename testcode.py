...
import streamlit as st
from passlib.hash import bcrypt
from db.db_utils import add_user, get_regions, get_all_locations, delete_locations
...
with st.form("signup_form"):
    ...
    phone = st.text_input("Phone Number")
    password = st.text_input("Password", type="password")
    role = st.selectbox("Role", ["merchandiser", "recruiter", "admin"])
    regions = get_regions()
    region_options = [r['name'] for r in regions]
    selected_region = st.selectbox("Select Region of Work", region_options)
    submit_button = st.form_submit_button("Create User")

    if submit_button:
        if not (email and phone and password and selected_region):
            st.error("Please fill all fields and select a region.")
        else:
            password_hash = bcrypt.hash(password)
            result = add_user(email, phone, password_hash, role, selected_region)
            if result is not None:
                st.success("User created successfully!")
            else:
                st.error("User creation failed: Email already exists.")
