...
def upload_image(image_bytes, folder='images', gps_lat=None, gps_long=None):
    try:
        ...
        if original_format == 'PNG':
            img.save(buffer, format="PNG", optimize=True)  # Lossless
        else:
            img.save(
                buffer,
                format="JPEG",
                quality=100,  # Maximum quality
                optimize=True,  # Strip metadata
                progressive=True  # Web-friendly loading
            )
        buffer.seek(0)

        key = f"{folder}/{uuid.uuid4()}.{original_format.lower()}"
