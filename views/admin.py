...
with setup_fields:
        ...
        ["Regions","Locations","SKUs","POSMs","Users","Location By Region", "Banks"], key="admin_setup_section")
...
if selection == "Location By Region":
    ...
    location, location_table=st.columns([4,3],gap='medium',vertical_alignment='top')
    with location:
        with st.expander("Manage Locations By Region", expanded=True):
            regions = execute_query("SELECT * FROM region")
            ...
            with location_table:
                try:
                    ...
                    locsbyregion = execute_query("SELECT * FROM locations_by_region WHERE region_id = %s", (region_id,))
                    st.dataframe(locsbyregion)
                    st.caption("Click on a location to view its outlets", unsafe_allow_html=True)
                except Exception as e:
                    st.error(f"Error fetching locations: {e}")
